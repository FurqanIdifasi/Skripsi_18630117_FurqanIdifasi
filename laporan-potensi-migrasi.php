...
<body class="A4 landscape">
  ...
  <section class="sheet padding-10mm " style="height: auto;font-size: 10px;">
    ...
    <h4 class="text-center">Laporan Potensi Migrasi  <?= date("M", strtotime($_GET['bulan']));  ?> <?= date("y", strtotime($_GET['bulan']));  ?></h4>
<hr>
          ...
          <tbody>
           ...
           /*   if($_SESSION['hak_akses'] == 'pelanggan') { 
                 $id = $_SESSION['user_id'];
                  ...
                  $datas = mysqli_query($koneksi, "select pemasangan.*, pendaftaran.no_psb, pendaftaran.tgl_psb, pelanggan.nama,pelanggan.hp,pelanggan.alamat,pelanggan.status_rumah  , karyawan.nama as nama_teknisi from pemasangan JOIN pendaftaran ON pendaftaran.id = pemasangan.pendaftaran_id JOIN pelanggan ON pelanggan.id = pendaftaran.pelanggan_id LEFT JOIN karyawan ON karyawan.id = pemasangan.teknisi_id WHERE pelanggan.status_rumah = 'SEWA/KONTRAK' AND pemasangan.status_pasang = 'SELESAI' AND YEAR(pemasangan.tgl_rencana_pasang) = '$tah' AND MONTH(pemasangan.tgl_rencana_pasang) = '$bul' group by pemasangan.id") or die(mysqli_error($koneksi)); 
             /* }*/
